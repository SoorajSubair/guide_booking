...
function ChatComponent() {
  ...
  return (
    <div className='chat-container'>
        {!isListClicked &&
        <ChatList isSmallScreen={isSmallScreen} setIsListClicked={setIsListClicked} setIsSmallScreen={setIsSmallScreen}/>
        }
        {chatId &&
        <div className={isSmallScreen ? "chat-window-container-small-device" : "chat-window-container"}>
            ...
            <div className="chat-window-header">
                ...
                <img src={`${baseUrl}${chat?.guide?.image}`} alt="" className="chat-window-header-avatar" />
                <h3 className="chat-window-header-h3">{chat?.guide?.first_name}</h3>
            </div>
                ...
                <div ref={messagesEndRef}>
                    <ul className="chat-window-ul" >
                        {[...new Set(messages.map((message) => message.id))].map((messageId) => {
                            ...
                            <div className={message.senderId === id ? "chat-window-message" : "chat-window-message-sender"} >
